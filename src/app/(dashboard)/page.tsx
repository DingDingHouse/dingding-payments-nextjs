...
import { Coins, FileText, LogOut, Sparkles, Star, Wallet, Wallet2 } from "lucide-react";
import Link from "next/link";
import { getQRCodes, getWalletsByType, getWalletTypes } from "./wallets/actions";
                        ...
                        <>
                            ...
                            <div className="px-6 pt-6 overflow-x-auto pb-2">
                                <div className="flex gap-3 mb-6 min-w-max">
                                    {walletTypes.map((walletType: WalletType) => (
                                        <Link
                                            key={walletType._id}
                                            href={`/?type=${walletType._id}${selectedWalletId && walletType._id === selectedWalletTypeId ? `&id=${selectedWalletId}` : ''}`}
                                            className={`flex items-center gap-2 px-4 py-2 rounded-xl whitespace-nowrap transition-all ${selectedWalletTypeId === walletType._id
                            ...
                            {!hasWallets ? (
                                ...
                                <div className="px-6 pb-6 text-center py-10">
                                    ...
                                    <h2 className="text-xl font-bold text-white mb-2">No Wallets Available</h2>
                                    <p className="text-[#8EACCD] max-w-md mx-auto mb-6">
                                        There are currently no wallets available for this payment method.
                                    </p>
                                    <TawkWrapper>
                                        <Button className="bg-[#2C73D2] hover:bg-[#2C73D2]/80 text-white">
                                            Contact Support
